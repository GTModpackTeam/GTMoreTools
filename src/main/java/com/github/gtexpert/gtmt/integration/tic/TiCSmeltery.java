...
import com.github.gtexpert.gtmt.integration.tic.api.SmelteryHelper;
 ...
 *
 * <p>
 * Processed forms (ingot, nugget, block, gem, dust variants) are always registered.
 * Raw ore and crushed ore registration is controlled by {@link TiCConfigHolder#smelteryOreMelting}.
 * Whether raw ores yield double ingots is controlled by {@link TiCConfigHolder#smelteryOreDoubling};
 * crushed ore variants always yield 1x regardless of that flag.
 */
public final class TiCSmeltery {
    ...
    /**
     * Called during material registration to add smeltery melting recipes for all
     * GT materials that carry a fluid property.
     */
    public static void register() {
        for (Material gtMaterial : GregTechAPI.materialManager.getRegisteredMaterials()) {
            ...
            if (TiCConfigHolder.smelteryOreMelting) {
                int oreAmount = TiCConfigHolder.smelteryOreDoubling ? VALUE_Ingot * 2 : VALUE_Ingot;
     ...
     *
     * <p>
     * Uses {@link OreDictUnifier#get(OrePrefix, Material)} to verify the item exists
     * before registering. Uses {@link MeltingRecipe} directly to specify the exact fluid
     * output amount, bypassing TiC's global {@code oreToIngotRatio} setting.
     *
     * <p>
     * External callers should use {@link SmelteryHelper#registerRecipe}
